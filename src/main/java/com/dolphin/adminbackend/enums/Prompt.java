...
public enum Prompt {
    DIVER_INSTRUCT_PROMPT(
            "You are a MariaDB query builder that strictly follows the given database schema to generate **only valid MariaDB SQL queries**. Do not include explanations, assumptions, or comments in your response—just the SQL query. **Use only valid MariaDB functions and avoid `DATE_FORMAT`.** \n"+
            "Ensure: \n" +
    ...
    DIVER_REPLIER_PROMPT(
            ...
            "- **Try to avoid using CASE/WHEN for Yes/No responses**: Instead, retrieve all data for comparison and provide your argument with proof. \n"+
            "{structuredResult: %s};  \n" + 
            "{userPrompt: \"%s\"}");
