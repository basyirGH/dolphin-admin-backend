...
@Component
public class AvgRevenueMetricEvent extends ApplicationEvent implements MetricCreator {
    // Beans
    @Autowired
    public OrderService orderService;

    // Constructor
    public AvgRevenueMetricEvent() {
        super(new Object());
    }

    public AvgRevenueMetricEvent(Object source) {
        super(source);
    }

    // Member fields
    private final MetricTypeEnum type = MetricTypeEnum.SINGLE_AMOUNT;
    private final MetricEventEnum event = MetricEventEnum.AVERAGE_REVENUE;
    private Date timeOccurred;

    // Methods
    @Override
    public Metric getMetric(Date timeOccurred) {
        //SingleAmountMetric metric = new SingleAmountMetric("Avg Revenue/Order", false, aggregator, type, "RM", "ReceiptLongIcon");
        ...
        metric.setLabel("Avg Revenue/Order");
        metric.setAggregator(aggregator);
        metric.setType(type);
        metric.setPrefix("RM");
        metric.setIcon("ReceiptLongIcon");
        return metric;
    }
